...
if 'messages_clients' not in st.session_state:
    st.session_state['messages_clients'] = [{"Client":"Admin","Message":"Bienvenue sur AgriMind","Date":datetime.now().strftime("%Y-%m-%d %H:%M")}]
...
def generer_rapport(pays,plante,temp_actuelle,pluie_actuelle):
    ...
    # Ajouter alerte popup et session
...
with tabs[3]:
    ...
    if st.button("Analyser silo"):
        ...
        if cluster==0:
            ...
            st.session_state['alertes'].append({"Date":datetime.now().strftime("%Y-%m-%d %H:%M"), 
                                                "Pays":"Stock", "Plante":"-", "Temp":temp,
                                                "Statut":"⚠️ Stock instable","Action":"Ajuster ventilation / humidité"})
        else:
            st.error("Risque fermentation ! Conseils : Vérifier température et CO2.")
            st.toast("🚨 Risque fermentation !",icon="⚠️")
            st.session_state['alertes'].append({"Date":datetime.now().strftime("%Y-%m-%d %H:%M"), 
                                                "Pays":"Stock", "Plante":"-", "Temp":temp,
                                                "Statut":"🚨 Risque fermentation","Action":"Vérifier température et CO2"})
...
# --------------------------
with tabs[5]:
    st.header("Messagerie clients")
    st.subheader("Messages reçus")
    if st.session_state['messages_clients']:
        for m in reversed(st.session_state['messages_clients']):
            color = "#DCF8C6" if m["Client"].lower()=="admin" else "#FFFFFF"
            border = "1px solid #ddd" if m["Client"].lower()!="admin" else "none"
            st.markdown(f"<div style='background-color:{color}; padding:8px; border-radius:10px; margin:5px 0; border:{border};'><b>{m['Client']}</b> [{m['Date']}]: {m['Message']}</div>", unsafe_allow_html=True)
    else:
        st.info("Aucun message pour le moment.")

    st.subheader("Envoyer un message")
    client = st.text_input("Nom du client ou 'Admin'", key="msg_client_input")
    msg = st.text_area("Message", key="msg_client_text")
    if st.button("Envoyer", key="send_client_msg"):
        if client and msg:
            ...
            st.session_state["msg_client_input"] = ""
            st.session_state["msg_client_text"] = ""
